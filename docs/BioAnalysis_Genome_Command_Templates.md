...
## 01 Preprocessing: raw-read filtering and NT-based contig decontamination

### RNA-seq read filtering with fastp
...
### WGS read filtering with fastp

```bash
bash scripts/01_preprocessing/run_fastp_wgs_read_filtering_workflow.sh \
  ...
  --qualified-quality-phred 15 \
  --length-required 50 \
  --threads 10
```

### RNA-seq read filtering with Trimmomatic

```bash
bash scripts/01_preprocessing/run_trimmomatic_rna_read_filtering_workflow.sh \
  --sample Arabidopsis_thaliana.flower \
...
### WGS read filtering with Trimmomatic
...
### NT-based contig decontamination
...
### Local EnrichPipeline GO/KEGG/IPR enrichment

Do not commit `EnrichPipeline.tar.gz` or extracted database files. Keep the external EnrichPipeline directory and databases outside the repository, and pass their paths at runtime.
...
## 10 HGT screening, full-method event summaries, and validation handoff

Run NR taxonlist DIAMOND searches when taxon-group NR outputs are not already available:

```bash
bash scripts/10_hgt/run_hgt_nr_taxonlist_workflow.sh \
...
Run the single-species blast2hgt screening and validation handoff:
...
Build comparable full-species Condition1/Condition2 matrices from a manifest of `.rp.tsv` inputs:
...
python3 scripts/10_hgt/build_hgt_condition_matrices.py \
  ...
  --prefix tree_species \
  --donor-groups bacteria,fungi,Metazoa,virus,archaea,other \
  --exclude-groups Plant,Viridiplantae,Rhodophyta,Glaucocystophyceae,unknown
```

Prepare HGT gene-source IDs for HGT-only OrthoFinder clustering:
...
Summarize HGT-driven node events with the preferred family-centric method:
...
Prepare plot-ready species donor and node-event handoff tables:
...
Integrate single-species HGT candidates with donor taxonomy, orthogroups, target-family evidence, and family-evolution results:
