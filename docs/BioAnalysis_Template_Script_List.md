...
| 02 assembly | `scripts/02_assembly/run_genome_survey_workflow.sh` | Reference-derived; cleaned wrapper | Short-read FASTQ/FASTA files | Jellyfish k-mer database, histogram, GenomeScope2 output, optional Smudgeplot output, and manifest |
| 02 assembly | `scripts/02_assembly/run_ploidyngs_workflow.sh` | Reference-derived; cleaned wrapper | Coordinate-sorted WGS BAM | ploidyNGS table, histogram PDF, runtime log, and manifest |
