...
| 03 Repeat annotation | Genome FASTA and repeat tools | `scripts/03_repeat/run_repeat_annotation_workflow.sh` plus Stage 03 helper scripts | Uppercase genome, repeat libraries, RepeatMasker GFF3/summary when supplied, TRF outputs | LTR and RepeatModeler outputs are non-empty when expected; repeat coordinates are valid | Missing `.scn`, missing library, unmerged RepeatMasker outputs |
