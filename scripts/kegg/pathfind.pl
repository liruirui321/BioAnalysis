...
my ($fg, $bg, $koMap, $kegg, $mapTitle,$output, $help);
...
options:
        ...
        -komap: ko_map.tab file
        -kegg:  kegg fasta
        -maptitle:      kegg map title file
        -output: output file, default is out.path
        -help: help information
e.g.:
        perl $0 -fg 1.ko -bg 2.ko -output out.path -kegg animal.fa -komap animal_komap.tab -maptitle map_title.tab
USAGE
}
...
#       print OUT "#Pathway\tCount (" . $sum1 . ")\tPathway ID\tGenes\tKOs\n";
        ...
        close PT;
#       @pValues = split /\n/, `/opt/blc/genome/biosoft/R/bin/Rscript ptemp.R | awk '{print \$2}' 2> /dev/null`;
#       @pValues = split /\n/, `$Bin/Rscript ptemp.R | awk '{print \$2}' 2> /dev/null`;
        @pValues = split /\n/, `/usr/local/bin/Rscript $name.ptemp.R | awk '{print \$2}'` or die;
        $p = join(", ", @pValues);
        open QT, "> $name.qtemp.R" || die $!;
        print QT <<QT;
library(qvalue)
p <- c($p)
q <- qvalue(p, lambda = 0)
...
#       @qValues = split /\n/, `$Rscript qtemp.R 2> /dev/null | awk 'NR > 1 {for (x = 2; x <= NF; x++) print \$x}'`;
        @qValues = split /\n/, `$Rscript $name.qtemp.R | awk 'NR > 1 {for (x = 2; x <= NF; x++) print \$x}'` or die;
        unlink "$name.ptemp.R", "$name.qtemp.R";
        ...
        for $pwID (sort {$stats{$a}{"pvalue"} <=> $stats{$b}{"pvalue"}} keys (%stats)) {
                $num1 = @{$stats{$pwID}{"genes"}};
                $num2 = @{$stats{$pwID}{"genes2"}};
                die "somewhere erro!\n" if( ! $stats{$pwID}{'pvalue'});
                die "somewhere erro!\n" if( ! $stats{$pwID}{'qvalue'});
...
#       print OUT "#Pathway\tSample1 (" . $sum1 . ")\tSample2 (" . $sum2 . ")\tPvalue\tQvalue\tPathway ID\tGenes\tKOs\n";
        print OUT "#Pathway\t$name (" . $sum1 . ")\tAll-gene (" . $sum2 . ")\tPvalue\tQvalue\tPathway ID\tLevel 1\tLevel 2\tGenes\tKOs\n";
        print OUT $content;
        close OUT;
}
