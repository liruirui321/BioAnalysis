...
usage() {
  cat <<'USAGE'
Usage: run_pandepth_coverage_workflow.sh --genome genome.fa --outdir pandepth_qc [read mode options]

Run an independent Stage 02 PanDepth assembly coverage workflow:
  1. Map WGS paired reads with BWA or long reads with minimap2.
  2. Sort and index the alignment BAM.
  3. Build sliding windows and run PanDepth coverage summaries.
...
Options:
  --output-name NAME               Output prefix [pandepth]
  --split-index TEXT               minimap2 split index size [8G]
  --window INT                     Window size [100000]
  --step INT                       Step size [25000]
  ...
  --pandepth CMD                   PanDepth executable [pandepth]
  ...
  --pandepth-option TEXT           Additional PanDepth option string; repeatable
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --pandepth) pandepth_cmd=${2:?"Missing value for --pandepth"}; shift 2 ;;
    ...
    --pandepth-option) pandepth_options+=("${2:?"Missing value for --pandepth-option"}"); shift 2 ;;
...
required_cmds=("$samtools_cmd" "$bedtools_cmd" "$pandepth_cmd")
if [[ ${#wgs_r1[@]} -gt 0 ]]; then
...
workdir="$outdir_abs/${output_name}.pandepth_work"
mkdir -p "$workdir"
...
stable_bam="$outdir_abs/${output_name}.pandepth.sorted.bam"
windows_bed="$outdir_abs/${output_name}.pandepth.windows.bed"
runtime_log="$outdir_abs/${output_name}.pandepth_running_time.txt"
read_mode="NA"
if [[ ${#wgs_r1[@]} -gt 0 ]]; then
  read_mode="wgs"
elif [[ ${#hifi[@]} -gt 0 ]]; then
...

{
  ...
  "$samtools_cmd" faidx "$work_genome"
  "$bedtools_cmd" makewindows -g "$work_genome.fai" -w "$window" -s "$step" > "$windows_bed"
  pandepth_args=(-i "$sorted_bam" -b "$windows_bed" -o "$outdir_abs/${output_name}.pandepth" -t "$threads")
  for option in "${pandepth_options[@]}"; do
    read -r -a option_parts <<< "$option"
    pandepth_args+=("${option_parts[@]}")
  done
  "$pandepth_cmd" "${pandepth_args[@]}"
...
cat > "$outdir_abs/${output_name}.pandepth_manifest.tsv" <<MANIFEST
field	value
