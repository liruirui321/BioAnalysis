...
my (%hash,%hash2,%pos,%pos2,%count);

while(<IN>){
    chomp;
    my @a=split/\s+/,$_;
	my $meth_level=$a[3]/($a[3]+$a[4]);
	$pos{$a[0]}{$a[1]}=$meth_level;
}
#print Dumper(\%pos);
...
foreach my $key_chr (sort keys %pos){
	...
	foreach my $key_pos (sort {$a<=>$b} keys %$pos2){
		my $step=int($key_pos/100000);
		$hash{$key_chr}{$step}+=$pos{$key_chr}{$key_pos};
		$count{$key_chr}{$step}++;
	}
}
...
foreach my  $key1 (sort keys %hash){
	...
	my $start=$key2*100000;
	my $end=$start+99999;
	my $percentage=sprintf("%.3f",$hash2->{$key2}/$count{$key1}{$key2});
