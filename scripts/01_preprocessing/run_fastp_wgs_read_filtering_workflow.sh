...
usage() {
  cat <<'USAGE'
Usage: run_fastp_wgs_read_filtering_workflow.sh --sample sample --r1 WGS_R1.fq.gz --outdir wgs_fastp [options]

Run an independent Stage 01 fastp WGS raw-read filtering workflow:
  1. Optionally run FastQC before filtering.
  2. Run fastp on single-end or paired-end WGS reads.
...
out_r1="$outdir_abs/${sample}.wgs.clean_R1.fastq.gz"
out_r2="$outdir_abs/${sample}.wgs.clean_R2.fastq.gz"
merged_out="$outdir_abs/${sample}.wgs.merged.fastq.gz"
html_report="$outdir_abs/${sample}.wgs.fastp.html"
json_report="$outdir_abs/${sample}.wgs.fastp.json"
fastp_log="$outdir_abs/${sample}.wgs.fastp.log"
runtime_log="$outdir_abs/${sample}.wgs_fastp_running_time.txt"
manifest="$outdir_abs/${sample}.wgs_fastp_manifest.tsv"
...
run_fastqc() {
  ...
  "$fastqc_cmd" "${args[@]}" > "$outdir_abs/${sample}.wgs.fastqc_${label}.log" 2>&1
}
