...
usage() {
  cat <<'USAGE'
Usage: run_trimmomatic_wgs_read_filtering_workflow.sh --sample sample --outdir wgs_trimmomatic [read mode options]

Run an independent Stage 01 Trimmomatic WGS raw-read filtering workflow:
...
runtime_log="$outdir_abs/${sample}.wgs_trimmomatic_running_time.txt"
summary="$outdir_abs/${sample}.wgs.trimmomatic.summary"
work_script="$outdir_abs/${sample}.wgs_trimmomatic_work.sh"
manifest="$outdir_abs/${sample}.wgs_trimmomatic_manifest.tsv"
adapter_abs=""
...
if [[ -n "$pe1" ]]; then
  ...
  out_1_paired="$outdir_abs/${sample}.wgs_1_paired.fq.gz"
  out_1_unpaired="$outdir_abs/${sample}.wgs_1_unpaired.fq.gz"
  out_2_paired="$outdir_abs/${sample}.wgs_2_paired.fq.gz"
  out_2_unpaired="$outdir_abs/${sample}.wgs_2_unpaired.fq.gz"
  ...
  out_single="$outdir_abs/${sample}.wgs.fq.gz"
